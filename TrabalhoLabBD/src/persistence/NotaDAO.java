...
public class NotaDAO {
	...
	public String[] carregarQuesitos() throws SQLException {
		...
		PreparedStatement ps = c.prepareStatement("SELECT nome FROM quesito");
		ResultSet rs = ps.executeQuery();
		while(rs.next()) {
			quesitos[i] = rs.getString("nome");
			i++;
		}
		return quesitos;
	}
	...
	public void carregarJurados(JComboBox<String> combo, int quesito) throws SQLException {
		...
		sql.append("SELECT ju.id_jurado FROM jurado ju ");
		sql.append("INNER JOIN juri jur ");
		sql.append("ON ju.id_jurado = jur.id_j ");
		sql.append("WHERE jur.id_q = ?");
	...
	public List<Quesito> notasQuesito(int quesito) throws SQLException {
		...
		while(rs.next()) {
			...
			q.setTotal(rs.getFloat("valor_total"));
			notas.add(q);
		}
		return notas;
	}
...
//		String sql = "CALL sp_verQuesito";
		StringBuffer sql = new StringBuffer();
		sql.append("select nome_escola as nome, total_pontos as total ");
